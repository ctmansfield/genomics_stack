    ...
    with p.open("r", encoding=enc, errors="ignore") as f:
        ...
        for line in f:
            ...
            if len(parts) < len(cols):
                continue
            rec = dict(zip(cols, parts, strict=False))
            ...
            genotype = (
                rec.get("genotype") if use_geno else ((allele1 or "") + (allele2 or "") or None)
            )
...
def process_upload(row) -> int:
    ...
    with get_con() as con:
        with con.cursor() as cur:
            ...
            if inserted_total > 0:
                cur.execute(
                    "update uploads set status='parsed', notes=coalesce(notes,'') || %s where id=%s",
                    (f" parsed_rows={inserted_total};", up_id),
                )
            else:
                cur.execute(
                    "update uploads set status='no_rows', notes=coalesce(notes,'') || %s where id=%s",
    ...
    while True:
        try:
            ...
            try:
                ...
                with get_con() as con:
                    con.execute(
                        "update uploads set status='error', notes=left(%s,1000) where id=%s",
