#!/usr/bin/env python3
import argparse, subprocess, sys, json, time, gzip, os, shutil, tempfile
...
def parse_args():
    ap = argparse.ArgumentParser(description="Resilient VEP annotator with schema-aligned TSV output.")
...
def extract_columns_from_vep_record(rec: Dict[str, str]) -> List[str]:
    ...
    ordered = [
        ...
        af, af_afr, af_amr, af_eas, af_eur, af_sas,
        clin_sig, sift, polyphen,
        cadd_raw, cadd_phred,
        json.dumps(extra, separators=(",",":"))
    ]
    if len(ordered) != len(REQUIRED_COLUMNS):
        raise RuntimeError(f"Column count mismatch: got {len(ordered)} expected {len(REQUIRED_COLUMNS)}")
    return ordered

def write_header(fp): fp.write("\t".join(REQUIRED_COLUMNS) + "\n")
...
def run_vep_on_chunk(lines: List[str], args) -> List[List[str]]:
    with tempfile.TemporaryDirectory() as td:
        ...
        cmd = [
            args.vep_path, "--offline",
            "--assembly", args.assembly,
            "--dir_cache", args.cache_dir,
            "--fasta", str(Path(args.ref_dir) / f"{args.assembly}.fa"),
            "--everything",
            "--fork", str(args.forks),
            "--input_file", str(chunk_vcf),
            "--format", "vcf",
            "--vcf", "--tab",
            "--fields",
        ...
        if code != 0: raise RuntimeError(f"VEP failed (exit {code}): {err[:1000]}")
        rows, vep_header = [], None
        for ln in out.splitlines():
            if ln.startswith("#"): continue
            if ln.startswith("Location\t"):
                vep_header = ln.strip().split("\t"); continue
            parts = ln.strip().split("\t")
            rec = dict(zip(vep_header, parts))
            extra = {}
            if "Extra" in rec:
...
def process_chunk(chunk_lines, out_fp, args):
    ...
    while True:
        try:
            ...
            for r in rows: out_fp.write("\t".join(r) + "\n")
            return len(rows)
        except Exception:
            tries += 1
            if tries > args.max_retries: raise
...
def main():
    ...
    produced = 0
    if out_tmp.exists() and not args.resume: safe_unlink(out_tmp)
    with open(out_tmp, "a") as out:
        if out.tell() == 0: write_header(out)
        chunk = []
        for line in vcf_iter(vcf):
            chunk.append(line)
            if len(chunk) >= args.chunk_size:
                produced += process_chunk(chunk, out, args); chunk = []
        if chunk: produced += process_chunk(chunk, out, args)
