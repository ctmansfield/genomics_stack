...
def main():
    ...
    out_dir = env("REPORT_OUT", "/root/genomics-stack/risk_reports/out")
    ...
    with psycopg2.connect(dsn) as conn, conn.cursor() as cur:
        ...
        if cur.fetchone()[0]:
            cur.execute(
                ...
                (args.file_id, args.file_id, args.file_id),
            )
        ...
        SELECT i.{join_key} AS variant_id, i.*, v.*
          ...
          LEFT JOIN {vep_table} v
            ON v.{join_key}=i.{join_key} AND v.{vep_id_col}=i.{import_id_col}
