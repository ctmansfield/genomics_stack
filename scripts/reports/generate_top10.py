...
def main() -> None:
    ...
    with psycopg2.connect(dsn) as conn, conn.cursor() as cur:
        ...
        SELECT i.{import_id_col} AS file_id, i.{join_key} AS variant_id, i.*,
               v.gene, v.consequence, v.{score_col}
        FROM {import_table} i
        LEFT JOIN {vep_table} v
          ON v.{vep_id_col} = i.{import_id_col} AND v.variant_id = i.{join_key}
        WHERE i.{import_id_col} = %s
        ORDER BY v.{score_col} DESC NULLS LAST, i.{join_key}
        LIMIT %s
        """
        cur.execute(sql, (file_id, args.limit))
        cols = [d.name for d in cur.description]
        rows = cur.fetchall()

    out_path = os.path.join(out_dir, f"top{args.limit}_{file_id}.csv")
