...
SELECT subject_id, COUNT(DISTINCT sample_id) AS n_samples FROM core.samples GROUP BY subject_id;

CREATE OR REPLACE VIEW report.variant_risk AS
SELECT sample_id, chrom, pos, ref, alt, consequence, gene FROM core.annotations WHERE 1=0; -- placeholder until enrichment arrives
