...
-- Uses ON CONFLICT(column_list) instead of ON CONSTRAINT to avoid dependency on named constraints.
...
-- 2) Variants (ensure unique index exists, then upsert by column list)
...
-- 3) Risk rules with system_tag (unique on columns)
...
ON CONFLICT (gene_id, variant_id, short_title) DO NOTHING;
