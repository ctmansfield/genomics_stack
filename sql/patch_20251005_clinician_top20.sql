...
-- 1) Core entities for risk panel (idempotent)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid='public.risk_rules'::regclass AND conname='uq_risk_rules_def'
  ) THEN
...
-- 2) Add system_tag to risk_rules (idempotent) and index
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    EXECUTE 'ALTER TABLE public.risk_rules ADD COLUMN system_tag text';
    EXECUTE 'UPDATE public.risk_rules SET system_tag = ''General'' WHERE system_tag IS NULL';
    EXECUTE 'ALTER TABLE public.risk_rules ALTER COLUMN system_tag SET DEFAULT ''General''';
    EXECUTE 'ALTER TABLE public.risk_rules ALTER COLUMN system_tag SET NOT NULL';
  END IF;
END$$;
CREATE INDEX IF NOT EXISTS idx_risk_rules_system ON public.risk_rules(system_tag);

-- 3) Pair-awareness support (idempotent)
...
-- 4) Helper functions used in scoring and ranking (idempotent)
...
-- 5) Zygosity inference and risk hit recompute (idempotent)
