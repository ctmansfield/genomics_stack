...
def compute_scores(model: pd.DataFrame, genos: pd.DataFrame):
    ...
    top = top.sort_values("abs_contrib", ascending=False).head(10)[["rsid","weight","dosage","contrib"]]
    return dict(raw_score=raw, normalized_score=norm, coverage_pct=coverage, top_contributors=top, merged=merged)

def band(norm: float):
    if norm <= -0.33: return "Lower-than-average"
    if norm < 0.33: return "Average"
    return "Higher-than-average"

def draw_gauge(c, x, y, w, h, normalized: float):
    zone_w = w/3.0
    from reportlab.lib import colors
    c.setFillColor(colors.HexColor("#d9ead3")); c.rect(x,y,zone_w,h,stroke=0,fill=1)
    c.setFillColor(colors.HexColor("#fff2cc")); c.rect(x+zone_w,y,zone_w,h,stroke=0,fill=1)
    c.setFillColor(colors.HexColor("#f4cccc")); c.rect(x+2*zone_w,y,zone_w,h,stroke=0,fill=1)
    c.setStrokeColor(colors.black); c.rect(x,y,w,h,stroke=1,fill=0)
    px = x + (normalized+1.0)/2.0*w
    c.setStrokeColor(colors.black); c.line(px,y,px,y+h)
    for frac in [0.0,0.5,1.0]: tx = x+frac*w; c.line(tx,y,tx,y+5)
    c.setFont("Helvetica",8)
    c.drawString(x-5, y+h+3, "-1"); c.drawString(x+w/2-3, y+h+3, "0"); c.drawString(x+w-3, y+h+3, "1")

def render_pdf(out_path, sample_id, phenotype_name, scores):
    c = canvas.Canvas(out_path, pagesize=A4)
    W,H = A4; m = 20*mm
    c.setFont("Helvetica-Bold", 18); c.drawString(m, H-m, "Genetic Risk Report (Research Use Only)")
    ...
    c.setFont("Helvetica-Bold", 14); c.drawString(m, H-m-70, "Summary")
    norm = scores["normalized_score"]; band_lbl = band(norm)
    ...
    gx, gy = m, H-m-135; draw_gauge(c, gx, gy, 120*mm, 10*mm, norm)
    c.setFont("Helvetica-Bold", 12); c.drawString(m, gy-20, "Top Variant Contributors")
    c.setFont("Helvetica", 10); cols = ["RSID","Weight","Dosage","Contribution"]; col_w = [40*mm,30*mm,30*mm,40*mm]
    x0 = m; y0 = gy-35
    for i, col in enumerate(cols): c.drawString(x0 + sum(col_w[:i]) + 2, y0, col)
    y = y0 - 12
    for _, row in scores["top_contributors"].iterrows():
        vals = [str(row["rsid"]), f"{row['weight']:.3f}", f"{row['dosage']:.2f}", f"{row['contrib']:.3f}"]
        for i, v in enumerate(vals): c.drawString(x0 + sum(col_w[:i]) + 2, y, v)
        y -= 12
        if y < 60*mm: break
