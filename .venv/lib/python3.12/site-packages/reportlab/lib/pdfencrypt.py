...
def encryptionkey(password, OwnerKey, Permissions, FileId1, revision=None):
    ...
    if DEBUG >= 10:
        print(
            ...
            % tuple(
                [hexText(str(x)) for x in (password, OwnerKey, Permissions, FileId1, revision, key)]
            )
        )
    return key
...
def computeO(userPassword, ownerPassword, revision):
    ...
    if DEBUG >= 10:
        print(
            "computeO(%s,%s,%s)==>%s"
            % tuple([hexText(str(x)) for x in (userPassword, ownerPassword, revision, O)])
        )
    return O
    ...
    if DEBUG >= 10:
        print(
            ...
            % tuple(
                [
                    hexText(str(x))
                    for x in (key, objectNumber, generationNumber, string, revision, encrypted)
                ]
            )
        )
    return encrypted
...
def encryptCanvas(
    ...
    enc = StandardEncryption(
        userPassword, ownerPassword, canPrint, canModify, canCopy, canAnnotate, strength=strength
    )
