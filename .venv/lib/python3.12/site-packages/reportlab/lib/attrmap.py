        ...
        if aMap and name[0] != "_":
            ...
            if isinstance(value, DerivedValue):
                ...
                try:
                    ...
                    raise e.__class__(
                        f"{obj.__class__.__name__}.{name} {validate}({value!r})"
                    ) from e
                else:
                    if not r:
                        raise AttributeError(
                            f"Illegal assignment of {value!r} to {name} in class {obj.__class__.__name__}"
                        )
            else:
                ...
                if isinstance(prop, property):
                    ...
                    raise AttributeError(
                        "Illegal attribute '%s' in class %s" % (name, obj.__class__.__name__)
                    )
    prop = getattr(obj.__class__, name, None)
    if isinstance(prop, property):
        fset = getattr(prop, "fset", None)
