...
try:
    ...
    def rl_glob(
        pattern, glob=glob.glob, fnmatch=fnmatch.fnmatch, _RL_DIR=_RL_DIR, pjoin=os.path.join
    ):
...
def recursiveImport(modulename, baseDir=None, noCWD=0, debug=0):
    ...
    try:
        ...
        if debug:
            ...
            print(
                "+++++ os.paths.isfile(%s)=%s"
                % (ascii("./%s.py" % modulename), ascii(os.path.isfile("./%s.py" % modulename)))
            )
...
def open_for_read(name, mode="b"):
    ...
    if trustedHosts:
        ...
        trustedHosts = re.compile("".join(("^(?:", "|".join(map(xre, trustedHosts)), ")\\Z")))
...
def rl_isdir(pn, os_path_isdir=os.path.isdir, os_path_normpath=os.path.normpath):
    ...
    if not pn.endswith(os.sep):
        pn += os.sep
    return (
        len(list(filter(lambda x, pn=pn: x.startswith(pn), list(__rl_loader__._files.keys())))) > 0
    )


def rl_listdir(
    pn, os_path_isdir=os.path.isdir, os_path_normpath=os.path.normpath, os_listdir=os.listdir
):
...
class ImageReader:
    ...
    def check_pil_image_size(cls, im):
        ...
        if size > max_image_size:
            raise MemoryError(
                "PIL %s %s x %s image would use %s > %s bytes" % (m, w, h, size, max_image_size)
            )
