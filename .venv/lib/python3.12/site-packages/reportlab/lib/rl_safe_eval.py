...
class UntrustedAstTransformer(ast.NodeTransformer):
    ...
    def guard_iter(self, node):
        ...
        * generator expresions
        ...
        if isinstance(node.target, ast.Tuple):
            ...
            new_iter = ast.Call(
                func=ast.Name("__rl_getiter__", ast.Load()), args=[node.iter], keywords=[]
            )
    ...
    def gen_lambda(self, args, body):
        return ast.Lambda(
            args=ast.arguments(args=args, vararg=None, kwarg=None, defaults=[]), body=body
        )
    ...
    def visit_Attribute(self, node):
        ...
        if (
            ...
            and node.attr != "__"
            and not self.nameIsAllowed(node.attr, False)
        ):
    ...
    def visit_Assign(self, node):
        ...
        for target in reversed(node.targets):
            if isinstance(target, ast.Tuple):
                wrapper = ast.Assign(
                    targets=[target], value=self.protect_unpack_sequence(target, node.value)
                )
    ...
    def visit_Import(self, node):
        ...
        node = self.visit_children(node)
        new_node = ast.Call(
            func=ast.Name("__rl_add__", ast.Load()), args=[node.left, node.right], keywords=[]
        )
        copy_locations(new_node, node)
        return new_node
    ...
    def visit_BinOp(self, node):
        ...
        if isinstance(op, (ast.Mult, ast.Add, ast.Pow)):
            opf = (
                ...
                else "__rl_add__" if isinstance(op, ast.Add) else "__rl_pow__"
            )
            new_node = ast.Call(
                func=ast.Name(opf, ast.Load()), args=[node.left, node.right], keywords=[]
            )
            copy_locations(new_node, node)
            return new_node
        return node
...
class __RL_SAFE_ENV__:
    ...
    def __init__(self, timeout=None, allowed_magic_methods=None, allowed_magic_names=None):
        ...
        self.__rl_builtins__ = __rl_builtins__ = {
            _: __rl_missing_func__(_) for _ in dir(builtins) if callable(getattr(builtins, _))
        }
    ...
    def __rl_is_allowed_name__(self, name, crash=True):
        ...
        if isinstance(name, strTypes):
            if name not in self.allowed_magic_names and (
                name in __rl_unsafe__
                or (
                    name.startswith("__")
                    and name != "__"
    ...
    def __rl_safe_eval__(
        ...
        if allowed_magic_methods is not None:
            self.allowed_magic_methods = __fix_set__(
                allowed_magic_methods, __allowed_magic_methods__
            )
...
class __rl_safe_eval__:
    ...
    def __call__(
        ...
        return self.env.__rl_safe_eval__(
            ...
            __frame_depth__=2,
            allowed_magic_names=allowed_magic_names,
        )
...
def rl_extended_literal_eval(expr, safe_callables=None, safe_names=None):
    ...
    safe_test = (
        lambda n: isinstance(n, ast.Constant) or isinstance(n, ast.Name) and n.id in safe_names
    )
    ...
    def _convert(node):
        if isinstance(node, ast.Constant):
            ...
            and isinstance(node.left, (ast.Constant, ast.UnaryOp, astBinOp))
        ):
