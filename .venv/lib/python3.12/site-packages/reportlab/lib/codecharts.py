...
class CodeChartBase(Flowable):
    ...
    def drawLabels(self, topLeft=""):
        ...
        for col in range(self.charsPerRow):
            self.canv.drawCentredString(
                (col + 1.5) * self.boxSize, (self.rows + 0.25) * self.boxSize, self.formatByte(col)
            )
...
class CodeWidget(Widget):
    ...
    def draw(self):
        ...
        g = Group()
        g.add(
            Rect(self.x, self.y, self.width, self.height, fillColor=None, strokeColor=colors.black)
        )
        for x in range(16):
            for y in range(16):
                charValue = y * 16 + x
                if charValue > 32:
                    s = String(
                        self.x + x * dx, self.y + (self.height - y * dy), int2Byte(charValue)
                    )
                    g.add(s)
        return g
