        ...
        return (
            sum(
                (
                    ...
                    for f, t in _py_unicode2T1(text, [self] + self.substitutionFonts)
                )
            )
            * 0.001
            * size
...
if "sameFrag" in _py_funcs:
    ...
    def _py_sameFrag(
        f,
        g,
        _cmp=("fontName", "fontSize", "textColor", "rise", "us_lines", "link", "backColor", "nobr"),
    ):
        fdict = f.__dict__
        gdict = g.__dict__
        if "cbDefn" in fdict or "lineBreak" in fdict or "cbDefn" in gdict or "lineBreak" in gdict:
            return 0
        fg = fdict.get
        gg = gdict.get
