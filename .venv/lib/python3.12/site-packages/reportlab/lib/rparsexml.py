...
def parsexml0(
    ...
    # special case COMMENT
        ...
        if xmltext[endcomment] != ">":
            raise ValueError(
                "invalid comment: contains double dashes %s" % repr(xmltext[cursor : cursor + 20])
            )
        ...
        if "=" not in tagcontent:
            ...
            if noclose:
                raise ValueError(
                    "unclosed start tag %s" % repr(xmltext[firstbracket : firstbracket + 20])
                )
            cursor = startsearch
            ...
            while taglistindex < lasttaglistindex:
                ...
                try:
                    ...
                    raise ValueError(
                        "attvalue,attentry,attlist=" + repr((attvalue, attentry, attlist))
                    )
    ...
    while docontents is not None:
        ...
        # is it a close bracket?
            ...
            if nextclosebracket < nextopenbracket:
                raise ValueError(
                    "unclosed close tag %s" % repr(xmltext[nextopenbracket : nextopenbracket + 20])
                )
        ...
        # first record any intervening content
            ...
            (parsetree, cursor) = parsexml0(
                xmltext, startingat=nextopenbracket, toplevel=None, entityReplacer=entityReplacer
            )
            if parsetree:
                L.append(parsetree)
