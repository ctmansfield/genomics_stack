    ...
    for line in lines:
        ...
        if line[0:16] == "StartCharMetrics":
            ...
            try:
                ...
                raise ValueError(
                    "Header information error in afm %s: line='%s'" % (afmFileName, line)
                )
            try:
                right = int(right)
            except:
    ...
    for dirname in T1SearchPath:
        ...
        for possible in possibles:
            try:
                ...
                t, v, b = sys.exc_info()
                v.args = (
                    " ".join(map(str, v.args)) + ", while looking for faceName=%r" % faceName,
                )
                raise
...
def registerEncoding(enc):
    ...
    if enc.name in _encodings:
        ...
        if enc.isEqual(_encodings[enc.name]):
            ...
            raise FontError(
                'Encoding "%s" already registered with a different name vector!' % enc.name
            )
    else:
        ...
        if fontName not in _fonts:
            if faceName in _dynFaceNames:
                ...
                if not ofont._dynamicFont:
                    raise ValueError(
                        "Attempt to register fonts %r %r for face %r" % (ofont, font, faceName)
                    )
                else:
