...
def readJPEGInfo(image):
    ...
    unsupportedMarkers = [0xC3, 0xC5, 0xC6, 0xC7, 0xC8, 0xC9, 0xCA, 0xCB, 0xCD, 0xCE, 0xCF]
...
class _fusc:
    ...
    def encrypt(self, s):
        return self.__rotate(
            asciiBase85Encode("".join(map(chr, self.__fusc(list(map(ord, s)))))), self._n
        )

    def decrypt(self, s):
        return "".join(
            map(chr, self.__fusc(list(map(ord, asciiBase85Decode(self.__rotate(s, -self._n))))))
        )
