        ...
        if psfontname in fm:
            ...
            try:
                ...
                if fontObj._dynamicFont:
                    raise PDFError(
                        "getInternalFontName(%s) called for a dynamic font" % repr(psfontname)
                    )
    ...
    def Reference(self, obj, name=None):
        ...
        if hasattr(obj, __InternalName__):
            ...
            if name is not None and name != intname:
                raise ValueError(
                    "attempt to reregister object %s with new name %s" % (repr(intname), repr(name))
                )
...
class PDFObjectReference(PDFObject):
    ...
    def format(self, document):
        try:
            ...
            raise KeyError(
                "forward reference to %s not resolved upon final formatting" % repr(self.name)
            )
...
class PDFCrossReferenceSubsection(PDFObject):
    ...
    def format(self, document):
        ...
        for id in idsequence:
            ...
            if num in taken:
                raise ValueError(
                    "object number collision %s %s %s" % (num, repr(id), repr(taken[id]))
                )
        ...
        for index in range(nelts):
            ...
            if isinstance(elt, dict):
                ...
                try:
                    ...
                    raise ValueError(
                        "destination tree elt tuple should have two elts, got %s" % len(elt)
                    )
    ...
    def AnnotationDict(self, **kw):
        ...
        for name in d.keys():
            if name not in permitted:
                raise ValueError(
                    "%s bad annotation dictionary name %s" % (self.__class__.__name__, name)
                )
        return PDFDictionary(d)
...
class PDFDestinationFitR(PDFObject):
    ...
    def format(self, document):
        pageref = document.Reference(self.page)
        A = PDFArray(
            [pageref, PDFName(self.typename), self.left, self.bottom, self.right, self.top]
        )
        return format(A, document)
...
class PDFSeparationCMYKColor:
    def __init__(self, cmyk):
        ...
        if not isinstance(cmyk, CMYKColor):
            ...
            raise ValueError(
                "%s needs a CMYKColor argument with a spotName" % self.__class__.__name__
            )
        self.cmyk = cmyk
