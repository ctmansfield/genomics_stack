...
class TTFontParser:
    ...
    def getSubfont(self, subfontIndex):
        if self.fileKind != "TTC":
            raise TTFError(
                '"%s" is not a TTC file: use this method' % (self.filename, self.fileKind)
            )
        try:
    ...
    def readTableDirectory(self):
        try:
            ...
            raise TTFError(
                'Corrupt %s file "%s" cannot read Table Directory' % (self.fileKind, self.filename)
            )
        if self.validate:
            self.checksumTables()
    ...
    def readHeader(self):
        ...
        try:
            ...
            raise TTFError(
                '"%s" is not a %s file: can\'t read version' % (self.filename, self.fileKind)
            )
...
class TTFontMaker:
    ...
    def makeStream(self):
        ...
        # Header
        write(pack(">lHHHH", 0x00010000, numTables, searchRange, entrySelector, rangeShift))
            ...
            for unichar in range(256):
                if T[unichar] == 0:
                    ...
                    if last != -1:
                        ...
                        or SH[0].idRangeOffset + (unichar - SH[0].firstCode) >= entryCount
                    ):
                        glyph = 0
                    else:
...
class TTFont:
    ...
    def __init__(
        self, name, filename, validate=0, subfontIndex=0, asciiReadable=None, shapable=True
    ):
        """Loads a TrueType font from filename.
