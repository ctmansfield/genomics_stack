...
try:
    ...
    class BidiList(list):
        ...
        def __repr__(self):
            return (
                f"{self.__class__.__name__}({super().__repr__()},{self.__bidiV__},{self.__bidiL__})"
            )
    ...
    def innerBidiStrWrap(s, bidiV=-1, bidiL=-1):
        ...
        if type(s) is str:
            ...
            if klassName not in _bidiKS:
                NS = dict(
                    BidiStr=BidiStr, klassName=klassName, klass=s.__class__, deepcopy=deepcopy
                )
                exec(
                    f"""class {klassName}(klass,BidiStr):\n"""
                    """\tdef __new__(cls,s,bidiV=-1,bidiL=-1):\n"""
    ...
    def bidiStrWrap(s, orig):
        if not isinstance(orig, BidiStr):
            return s
        return innerBidiStrWrap(s, orig.__bidiV__, orig.__bidiL__)

    def bidiListWrap(L, orig):
        if not isinstance(orig, (BidiList, BidiIndex)) or isinstance(L, BidiList):
            return L
        if type(L) is list:
            klass = BidiList
        else:
            ...
            if klassName not in _bidiKS:
                NS = dict(
                    BidiList=BidiList, klassName=klassName, klass=L.__class__, deepcopy=deepcopy
                )
                exec(
                    f"""class {klassName}(klass,BidiList):\n"""
                    """\tdef __init__(self,L,bidiV=-1,bidiL=-1):\n"""
    ...
    def bidiShapedText(
        text, direction="RTL", clean=True, fontName="Helvetica", fontSize=10, shaping=False
    ):
        ...
        if direction in ("RTL", "LTR"):
            if shaping:
                ...
                SW = [
                    shapeStr(LW[i], fontName, fontSize) for i in VX
                ]  # shaped words in visual order
                if len(VX) > 1 and VX[0] > VX[-1]:
                    bL, bR = bR, bL
                text = shapeStr(bL * " ", fontName, fontSize, force=True)
                for w in SW:
                    if not hasattr(w, "__shapeData__"):
                        w = shapeStr(w, fontName, fontSize, force=True)
                    if w is not SW[0]:
                        text += shapeStr(" ", fontName, fontSize, force=True)
                    text += w
                if bR:
        ...
        width = (
            sum((_.x_advance for _ in text.__shapeData__)) * fontSize / 1000
        ...
        for i, (f, s) in enumerate(w[1:]):
            if hasattr(f, "cbDefn"):
                ...
                for u in s:
                    ...
                    text += u
        if len(text) <= 1:
            return w  # nothing to bidi
        fL = fL.__self__
        V2L = []
        bidi = log2vis(text, base_direction=direction, clean=clean, positions_V_to_L=V2L)
        fsL = [
            (w[1 + k][0], "".join((ks[1] for ks in g)))
            for k, g in groupby(((fL[V2L[i]], v) for i, v in enumerate(bidi)), key=itemgetter(0))
        ]
    ...
    bidiWordList = bidiStrWrap = bidiListWrap = bidiFragWord = innerBidiStrWrap = BidiIndex = (
        bidiText
    )
    rtlSupport = False
                            ...
                            if cluster is None or sd.cluster < 0 or r != self._rise:
                                if cluster is not None:
                                    ...
                                    A = [
                                        ...
                                        for v in (
                                            (
                                                ("(%s)" % canv_escape(b"".join(g)))
                                                if k
                                                else fp_str(sum(g))
                                            )
                                            for k, g in groupby(
                                                filter(None, A.__self__),
    ...
    def _shapedTextOut(self, text, dx, dy):
        ...
        for subset, t in font.splitString(text, canv._doc):
            if subset != self._curSubset:
                ...
                R.append(
                    "%s %s Tf %s TL" % (pdffontname, fp_str(self._fontsize), fp_str(self._leading))
                )
                self._curSubset = subset
            if dy:
                print(f"{dy} -->", end="")
