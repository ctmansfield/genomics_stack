    ...
    def set(self, canv, a, v):
        ...
        if isinstance(d, bool):
            ...
            if v not in self.allowed[a]:
                raise ValueError(
                    "ExtGstate[%r] = %r not in allowed values %r" % (a, v, self.allowed[a])
                )
...
class Canvas(_PDFColorSetter):
    ...
    def __init__(
        ...
        **kwds,
        ...
        self._initialLeading = (
            initialLeading if initialLeading is not None else self._initialFontSize * 1.2
        )
    ...
    def highlightAnnotation(
        ...
        **kw,
        ...
        self._addAnnotation(
            pdfdoc.HighlightAnnotation(Rect, contents, QuadPoints, Color, **kw), name, addtopage
        )

    def inkAnnotation(
        self, contents, InkList=None, Rect=None, addtopage=1, name=None, relative=0, **kw
    ):
    ...
    def linkURL(
        self, url, rect, relative=0, thickness=0, color=None, dashArray=None, kind="URI", **kw
    ):
    ...
    def rect(self, x, y, width, height, stroke=1, fill=0):
        ...
        self._code.append(
            "n %s re " % fp_str(x, y, width, height) + PATH_OPS[stroke, fill, self._fillMode]
        )
    ...
    def linearGradient(self, x0, y0, x1, y1, colors, positions=None, extend=True):
        ...
        shading = PDFAxialShading(
            x0, y0, x1, y1, Function=fcn, ColorSpace=colorSpace, Extend=_gradientExtendStr(extend)
        )
        self.shade(shading)
    ...
    def drawString(
        self, x, y, text, mode=None, charSpace=0, direction=None, wordSpace=None, shaping=False
    ):
        """Draws a string in the current text styles."""
        text, width = bidiShapedText(
            text, direction, fontName=self._fontname, fontSize=self._fontsize, shaping=shaping
        )
    ...
    def setFont(self, psfontname, size, leading=None):
        ...
        if not font._dynamicFont:
            if font.face.builtIn or not getattr(self, "_drawTextAsPath", False):
                ...
                self._code.append(
                    "BT %s %s Tf %s TL ET" % (pdffontname, fp_str(size), fp_str(leading))
                )
    ...
    def stringWidth(self, text, fontName=None, fontSize=None):
        ...
        return pdfmetrics.stringWidth(
            text, fontName or self._fontname, (fontSize, self._fontsize)[fontSize is None]
        )
