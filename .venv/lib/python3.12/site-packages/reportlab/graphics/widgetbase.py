...
class TypedPropertyCollection(PropHolder):
    ...
    def __getitem__(self, x):
        ...
        try:
            ...
            for i in filter(
                lambda x, K=list(child.__dict__.keys()): x in K, list(child._attrMap.keys())
            ):
                del child.__dict__[i]
            child.__dict__.update(
                dict(__propholder_parent__=weakref_ref(self), __propholder_index__=x[:-1])
            )
...
class Face(Widget):
    ...
    def draw(self):
        ...
        g.add(
            shapes.Polygon(
                points=[s * 0.5, s * 0.6, s * 0.4, s * 0.3, s * 0.6, s * 0.3], fillColor=None
            )
        )
...
class CandleStickProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        axes=AttrMapValue(
            SequenceOf(isString, emptyOK=0, lo=2, hi=2), desc="candle direction", advancedUsage=1
        ),
    )
        ...
        if haveLine:
            ...
            if not haveBox or crossLo >= boxHi or crossHi <= boxLo:
                ...
                if crossWidth is not None:
                    aLine(
                        position - crossWidth * 0.5, crossLo, position + crossWidth * 0.5, crossLo
                    )
                    aLine(
                        position - crossWidth * 0.5, crossHi, position + crossWidth * 0.5, crossHi
                    )
            elif haveBox:
                if crossLo < boxLo:
