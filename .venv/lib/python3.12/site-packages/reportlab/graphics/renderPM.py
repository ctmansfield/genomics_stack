...
class _PMRenderer(Renderer):
    ...
    def drawImage(self, image):
        ...
        self._canvas._aapixbuf(
            ...
            (
                im
                if self._canvas._backend == "rlPyCairo"  # rlPyCairo has a from_pil method
        ...
        if fill is not None or textRenderMode:
            ...
            if not text_anchor in ["start", "inherited"]:
                ...
                if text_anchor == "end":
                    ...
                    x -= numericXShift(
                        text_anchor, text, textLen, fontName, fontSize, stringObj.encoding
                    )
                else:
...
def _saveAsPICT(im, fn, fmt, transparent=None):
    ...
    s = _pmBackend.pil2pict(
        cols, rows, (im.tobytes if hasattr(im, "tobytes") else im.tostring)(), im.im.getpalette()
    )
...
class PMCanvas:
    def __init__(self, w, h, dpi=72, bg=0xFFFFFF, configPIL=None, backend=None, backendFmt="RGB"):
    ...
    def toPIL(self):
        im = _getImage().new(
            "RGBA" if self._backend == "rlPyCairo" and getattr(self, "_fmt") == "ARGB32" else "RGB",
            size=(self._gs.width, self._gs.height),
        )
        ...
        if fmt == "TIFF":
            ...
            if tc:
                ...
                im = Image.merge(
                    "RGBA",
                    im.split()
                    + (ImageChops.invert(im.point(T).convert("L").point(255 * [0] + [255])),),
                )
    ...
    def drawString(
        self, x, y, text, _fontInfo=None, text_anchor="left", direction=None, shaping=False
    ):
        gs = self._gs
        ...
        text, textLen = bidiShapedText(
            text, direction, fontName=fontName, fontSize=fontSize, shaping=shaping
        )
