...
class BarChart(PlotArea):
    ...
    _attrMap = AttrMap(
        ...
        barLabelArray=AttrMapValue(
            None, desc="explicit array of bar label values, must match size of data if present."
        ),
        reversePlotOrder=AttrMapValue(
            isBoolean, desc="If true, reverse common category plot order.", advancedUsage=1
        ),
        naLabel=AttrMapValue(
            NoneOrInstanceOfNA_Label, desc="Label to use for N/A values.", advancedUsage=1
        ),
        ...
        categoryLabelBarSize=AttrMapValue(
            isNumber, desc="width to leave for a category label to go between categories."
        ),
        categoryLabelBarOrder=AttrMapValue(
            OneOf("first", "last", "auto"), desc="where any label bar should appear first/last"
        ),
    ...
    annotations=7""",
        ...
        seriesOrder=AttrMapValue(
            SequenceOf(SequenceOf(isInt, emptyOK=0, NoneOK=0, lo=1), emptyOK=0, NoneOK=1, lo=1),
    ...
    def _drawBegin(self, org, length):
        ...
        return (
            crossesAt if vA.forceZero or (crossesAt >= org and crossesAt <= org + length) else org
        )
    ...
    def _addBarLabel(self, g, rowNo, colNo, x, y, width, height):
        ...
        if text:
            self._addLabel(
                text, self.barLabels[(rowNo, colNo)], g, rowNo, colNo, x, y, width, height
            )
        ...
        for rowNo, row in enumerate(BP):
            ...
            for colNo, (x, y, width, height) in enumerate(row):
                ...
                if isLine:
                    ...
                    if symbol:
                        sym = uSymbol2Symbol(
                            tpcGetItem(symbol, colNo), xL, yL, style.strokeColor or style.fillColor
                        )
                        if sym:
                            lineSyms(sym)
    ...
    def _computeMaxSpace(self, size, required):
        ...
        for i, ci in enumerate(C):
            ...
            if alo > ahi:
                ...
                maxS = ahi
                maxP = (
                    flipXY and (hi - ahi, v0, hi, v0 + size, 1) or (v0, hi - ahi, v0 + size, hi, 1)
                )
                if maxS >= required:
                    break
        return maxS, maxP
