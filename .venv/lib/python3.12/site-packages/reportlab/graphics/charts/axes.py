...
class CALabel(PMVLabel):
    _attrMap = AttrMap(
        ...
        labelPosFrac=AttrMapValue(
            isNumber, desc="where in the category range [0,1] the labels should be anchored"
        ),
    )
...
class CategoryAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        style=AttrMapValue(
            OneOf("parallel", "stacked", "parallel_3d"), "How common category bars are plotted"
        ),
...
class ValueAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        labels=AttrMapValue(None, desc="Handle of the axis labels."),
        labelAxisMode=AttrMapValue(
            OneOf("high", "low", "axis"), desc="Like joinAxisMode, but for the axis labels"
        ),
        ...
        keepTickLabelsInside=AttrMapValue(
            isBoolean, desc="Ensure tick labels do not project beyond bounds of axis if true"
        ),
        skipGrid=AttrMapValue(
            OneOf("none", "top", "both", "bottom"), "grid lines to skip top bottom both none"
        ),
    ...
    def _calcValueStep(self):
        ...
        if self.valueStep is None:
            ...
            rawInterval = rawRange / min(
                float(self.maximumTicks - 1), (float(self._length) / self.minimumTickSpacing)
            )
...
class NormalDateXValueAxis(XValueAxis):
    ...
    _attrMap = AttrMap(
        ...
        dailyFreq=AttrMapValue(
            isBoolean, desc="True if we are to assume daily data to be ticked at end of month."
        ),
        specifiedTickDates=AttrMapValue(
            NoneOr(SequenceOf(isNormalDate)), desc="Actual tick values to use; no calculations done"
        ),
        specialTickClear=AttrMapValue(
            isBoolean, desc="clear rather than delete close ticks when forced first/end dates"
        ),
        skipGrid=AttrMapValue(
            OneOf("none", "top", "both", "bottom"), "grid lines to skip top bottom both none"
        ),
    ...
    def _xAxisTicker(self, xVals):
        ...
        if self.specifiedTickDates:
            ...
            if (
                ...
                and (axisLength / float(ticks[-1] - ticks[0])) * (ticks[1] - ticks[0]) <= W
            ):
            ...
            if (
                self.forceEndDate
                and endDate == ticks[-1]
                and (axisLength / float(ticks[-1] - ticks[0])) * (ticks[-1] - ticks[-2]) <= W
            ):
                if self.specialTickClear:
                    labels[-2] = ""
                else:
