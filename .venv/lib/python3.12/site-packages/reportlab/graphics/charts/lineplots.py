...
class LinePlot(AbstractLineChart):
    ...
    _attrMap = AttrMap(
        ...
        lineLabelNudge=AttrMapValue(
            isNumber, desc="Distance between a data point and its label.", advancedUsage=1
        ),
        ...
        for rowNo, row in enumerate(data):
            if isinstance(row, FillPairedData):
                ...
                if 0 <= other < n:
                    ...
                    raise ValueError(
                        "data row %r is paired with invalid data row %r" % (rowNo, other)
                    )
            line = [].append
            for colNo, datum in enumerate(row):
                xv = datum[0]
                line(
                    (xscale(mktime(mkTimeTuple(xv))) if isStr(xv) else xscale(xv), yscale(datum[1]))
                )
            P(line.__self__)
        P = P.__self__
                    ...
                    if filler:
                        ...
                        inFillG.add(
                            Polygon(
                                fpoints,
                                fillColor=fillColor,
                                strokeColor=strokeColor if strokeColor == fillColor else None,
                                strokeWidth=width or 0.1,
                            )
                        )
                if not inFill or inFill == 2 or strokeColor != fillColor:
                    line = PolyLine(
                        points, strokeColor=strokeColor, strokeLineCap=0, strokeLineJoin=1
                    )
                    if width:
    ...
    def addCrossHair(self, name, xv, yv, strokeColor=colors.black, strokeWidth=1, beforeLines=True):
        ...
        annotations = [
            a for a in getattr(self, "annotations", []) if getattr(a, "name", None) != name
        ]
...
def _maxWidth(T, fontName, fontSize):
    ...
    return (
        T
        and max(list(map(lambda t, sW=stringWidth, fN=fontName, fS=fontSize: sW(t, fN, fS), T)))
        or 0
    )
...
class ScatterPlot(LinePlot):
    ...
    _attrMap = AttrMap(
        ...
        labelOffset=AttrMapValue(
            isNumber, desc="Space between label and Axis (or other labels)", advancedUsage=1
        ),
    ...
    def __init__(self):
        ...
        self.y = (
            self.bottomPadding + _axisTickLengths + _labelOffset + self.xValueAxis.labels.fontSize
        )
