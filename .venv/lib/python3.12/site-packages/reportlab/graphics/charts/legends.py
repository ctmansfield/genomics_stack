...
class SubColProperty(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        align=AttrMapValue(
            OneOf("left", "right", "center", "centre", "numeric"), desc="alignment in subCol"
        ),
...
class Legend(Widget):
    ...
    _attrMap = AttrMap(
        ...
        variColumn=AttrMapValue(
            isBoolean, "If true column widths may vary (default is false)", advancedUsage=1
        ),
        ...
        swatchCallout=AttrMapValue(
            None, desc="a user swatch callout(self,g,x,y,i,(col,name),swatch)", advancedUsage=1
        ),
    ...
    def _calculateMaxBoundaries(self, colorNamePairs):
        ...
        if self.variColumn:
            ...
            return [
                _transMax(n, M[r : r + columnMaximum]) for r in range(0, len(M), self.columnMaximum)
            ]
        else:
        ...
        for i in range(n):
            ...
            if isAuto(col):
                ...
                if isSymbol(swatchMarker):
                    c = uSymbol2Symbol(swatchMarker, swatchX + dx / 2.0, swatchY + dy / 2.0, col)
                else:
