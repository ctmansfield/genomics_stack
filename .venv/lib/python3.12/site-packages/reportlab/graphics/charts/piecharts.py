...
class WedgeProperties(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        shadingDirection=AttrMapValue(
            OneOf("normal", "anti"), desc="Whether shading is at start or end of wedge/sector"
        ),
...
class Pie(AbstractPieChart):
    _attrMap = AttrMap(
        ...
        data=AttrMapValue(
            isListOfNumbers, desc="List of numbers defining wedge sizes; need not sum to 1"
        ),
        labels=AttrMapValue(
            isListOfStringsOrNone, desc="Optional list of labels to use for each data point"
        ),
        ...
        other_threshold=AttrMapValue(
            isNumber, desc="A value for doing threshholding, not used yet.", advancedUsage=1
        ),
        ...
        sameRadii=AttrMapValue(
            isBoolean, desc="If true make x/y radii the same(default off)", advancedUsage=1
        ),
        ...
        sideLabels=AttrMapValue(
            isBoolean, desc="If true attempt to make piechart with labels along side and pointers"
        ),
                ...
                if not sideLabels:
                    ...
                    if text:
                        ...
                        if (
                            ...
                            labelX = (
                                (1 + self.sideLabelsOffset) * self.width + self.x + slices_popout
                            )
                            rx = 0
                        else:
...
class LegendedPie(Pie):
    ...
    _attrMap = AttrMap(
        ...
        legend_data=AttrMapValue(
            isNoneOrListOfNoneOrNumbers, desc="Numbers used on r/hand side of legend (or None)"
        ),
...
class Pie3d(Pie):
    ...
    def draw(self):
        ...
        for i in range(n):
            ...
            strokeColor = (
                _getShaded(style.strokeColor, style.strokeColorShaded, style.shading) or fillColor
            )
