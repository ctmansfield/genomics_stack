...
def split_arc_values(op, value):
    ...
    a_seq_re = (
        r"[\s,]*".join([float_re, float_re, float_re, flag_re, flag_re, float_re, float_re])
        + r"[\s,]*"
    )
    res = []
...
def end_point_to_center_parameters(x1, y1, x2, y2, fA, fS, rx, ry, phi=0):
    ...
    dtheta = (
        vector_angle(((x1d - cxd) / rx, (y1d - cyd) / ry), ((-x1d - cxd) / rx, (-y1d - cyd) / ry))
        % 360
    )
    ...
    def __init__(self, s, isClipPath=0, autoclose=None, fillMode=FILL_EVEN_ODD, **kw):
        ...
        for i in range(0, len(normPath), 2):
            ...
            # quadratic bezier, absolute
                ...
                (x0, y0), (x1, y1), (x2, y2), (xn, yn) = convert_quadratic_to_cubic_path(
                    (x0, y0), (x1, y1), (xn, yn)
                )
