...
class Drawing(Group, Flowable):
    ...
    _xtraAttrMap = AttrMap(
        ...
        background=AttrMapValue(
            isValidChildOrNone, desc="Background widget for the drawing e.g. Rect(0,0,width,height)"
        ),
        hAlign=AttrMapValue(
    ...
    _attrMap = AttrMap(
        ...
        formats=AttrMapValue(
            SequenceOf(OneOf(*_saveModes), lo=1, emptyOK=0), desc="One or more plot modes"
        ),
    )
    ...
    def resized(self, kind="fit", lpad=0, rpad=0, bpad=0, tpad=0):
        ...
        if kind == "fit" or (
            kind == "expand" and (xL < lpad or xH > oW - rpad or yL < bpad or yH > oH - tpad)
        ):
...
class LineShape(Shape):
    ...
    _attrMap = AttrMap(
        ...
        strokeOpacity=AttrMapValue(
            isOpacity, desc="The level of transparency of the line, any real number betwen 0 and 1"
        ),
        strokeOverprint=AttrMapValue(isBoolean, desc="Turn on stroke overprinting"),
        overprintMask=AttrMapValue(
            isBoolean, desc="overprinting for ordinary CMYK", advancedUsage=1
...
class Polygon(SolidShape):
    ...
    _attrMap = AttrMap(
        BASE=SolidShape,
        points=AttrMapValue(
            isListOfNumbers, desc="list of numbers in the form x1, y1, x2, y2 ... xn, yn"
        ),
    )
...
class Hatching(Path):
    ...
    _attrMap = AttrMap(
        ...
        xyLists=AttrMapValue(
            EitherOr((isListOfNumbers, SequenceOf(isListOfNumbers, lo=1)), "xy list(s)"),
...
class String(Shape):
    ...
    _attrMap = AttrMap(
        ...
        textRenderMode=AttrMapValue(
            OneOf(0, 1, 2, 3, 4, 5, 6, 7), desc="Control whether text is filled/stroked etc etc"
        ),
    )
    encoding = "utf8"
