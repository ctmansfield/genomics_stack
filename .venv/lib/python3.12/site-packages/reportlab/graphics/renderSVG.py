...
EXTRA_STROKE_STYLES = (
    "stroke-width stroke-linecap stroke stroke-opacity stroke-dasharray stroke-dashoffset".split()
)
...
class EncodedWriter(list):
    ...
    def write(self, u):
        if isBytes(u):
            ...
            raise ValueError(
                "EncodedWriter.write(%s) argument should be 'utf-8' bytes or str" % ascii(u)
            )
        self.append(u)
...
class SVGCanvas:
    def __init__(self, size=(300, 300), encoding="utf-8", verbose=0, bom=False, **kwds):
        ...
        self.style = {}
        self.path = ""
        self._strokeColor = self._fillColor = self._lineWidth = self._font = self._fontSize = (
            self._lineCap
        ) = self._lineJoin = None
    ...
    def drawString(
        self, s, x, y, angle=0, link_info=None, text_anchor="left", textRenderMode=0, **_svgAttrs
    ):
        if textRenderMode == 3:
            return  # invisible
        s = asNative(s)
        ...
        needFill = (
            textRenderMode == 0 or textRenderMode == 2 or textRenderMode == 4 or textRenderMode == 6
        )
        needStroke = (
            textRenderMode == 1 or textRenderMode == 2 or textRenderMode == 5 or textRenderMode == 6
        )
    ...
    def drawCentredString(
        self, s, x, y, angle=0, text_anchor="middle", link_info=None, textRenderMode=0, **_svgAttrs
    ):
        if self.verbose:
    ...
    def drawRightString(
        self, text, x, y, angle=0, text_anchor="end", link_info=None, textRenderMode=0, **_svgAttrs
    ):
        if self.verbose:
    ...
    def line(self, x1, y1, x2, y2):
        if self._strokeColor != None:
            if 0:  # something is wrong with line in my SVG viewer...
                line = transformNode(
                    self.doc, "line", x=x1, y=y1, x2=x2, y2=y2, style=self._formatStyle(LINE_STYLES)
                )
    ...
    def polyLine(self, points):
        ...
        if self._strokeColor != None:
            pts = ", ".join([fp_str(*p) for p in points])
            polyline = transformNode(
                self.doc, "polyline", points=pts, style=self._formatStyle(AREA_STYLES, fill=None)
            )
            self.currGroup.appendChild(polyline)
    ...
    def translate(self, x, y):
        if (x, y) != (0, 0):
            self.currGroup.setAttribute(
                "transform",
                "%s %s"
                % (self.currGroup.getAttribute("transform"), "translate(%s)" % self.cfp_str(x, y)),
            )
    ...
    def scale(self, sx, sy):
        if (sx, sy) != (1, 1):
            self.currGroup.setAttribute(
                "transform",
                "%s %s"
                % (self.groups[-1].getAttribute("transform"), "scale(%s)" % self.cfp_str(sx, sy)),
            )
