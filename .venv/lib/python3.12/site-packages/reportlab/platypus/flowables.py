...
class KeepInFrame(_Container, Flowable):
    def __init__(
        ...
        self.mode = mode
        assert mode in ("error", "overflow", "shrink", "truncate"), "%s invalid mode value %s" % (
            self.identity(),
            mode,
        )
...
class _FindSplitterMixin:
    def _findSplit(
        self, canv, availWidth, availHeight, mergeSpace=1, obj=None, content=None, paraFix=True
    ):
        """return max width, required height for a list of flowables F"""
        W = 0
        H = 0
...
class ImageAndFlowables(_Container, _FindSplitterMixin, Flowable):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        ...
        if self.imageHref:
            canv.linkURL(
                self.imageHref,
                (Ix, y + self.height - self._itpad - self._hI, Ix + self._wI, y + self.height),
                relative=1,
            )
...
class BalancedColumns(_FindSplitterMixin, NullDraw):
    ...
    def _generated_content(self, aW, aH):
        ...
        doVLines = (
            self._vLinesStrokeColor and self._vLinesStrokeWidth and self._vLinesStrokeWidth >= 0
        )
...
class FrameBG(AnchorFlowable):
    ...
    def __repr__(self):
        return "%s(%s)" % (
            self.__class__.__name__,
            ", ".join(
                ["%s=%r" % (i, getattr(self, i, None)) for i in "start color left right".split()]
            ),
        )
...
class FrameSplitter(NullDraw):
    ...
    def wrap(self, aW, aH):
        ...
        if aH < self.gap + self.required - _FUZZ:
            ...
            if not T:
                raise LayoutError(
                    "%s.nextTemplate=%s not found" % (self.identity(), self.nextTemplate)
                )
            T = T[0]
...
class DDIndenter(Flowable):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        self._flowable.drawOn(
            canv, x + self._leftIndent, y, max(0, _sW - self._leftIndent - self._rightIndent)
        )
...
class LIIndenter(DDIndenter):
    _IndenterAttrs = (
        "_flowable _bullet _leftIndent _rightIndent width height spaceBefore spaceAfter".split()
    )

    def __init__(
        self, flowable, leftIndent=0, rightIndent=0, bullet=None, spaceBefore=None, spaceAfter=None
    ):
        self._flowable = flowable
        self._bullet = bullet
        self._leftIndent = leftIndent
    ...
    def split(self, aW, aH):
        ...
        if len(S) > 1 and hasattr(f, "blPara"):
            ...
            if (
                getattr(getattr(f, "style", None), "alignment", None) == 4 and nnl >= (cnl - 2)
            ) or nnl == (cnl - 1):
                S = []
        return (
            [
        ...
        **kwds,
        ...
        if style:
            if not isinstance(style, ListStyle):
                raise ValueError(
                    "%s style argument (%r) not a ListStyle" % (self.__class__.__name__, style)
                )
            self._style = style
            ...
            if auto:
                for v in start:
                    if v not in self._numberStyles:
                        raise ValueError(
                            "invalid start=%r or bullettype=%r" % (start, self._bulletType)
                        )
            else:
