...
class paragraphEngine:
    ...
    def format(self, maxwidth, maxheight, program, leading=0):
        ...
        while remainder and room:  # heightremaining>=self.leading and remainder:
            ...
            try:
                (lineIsFull, line, cursor, currentLength, usedIndent, maxLength, justStrings) = (
...
class Para(Flowable):
    ...
    def __init__(
        self, style, parsedText=None, bulletText=None, state=None, context=None, baseindent=0
    ):
        ...
        self.style1 = (
            style  # make sure Flowable doesn't use this unless wanted! call it style1 NOT style
        )
    ...
    def wrap(self, availableWidth, availableHeight):
        ...
        if remainder:
            ...
            self.remainder = Para(
                self.style1, parsedText=None, bulletText=None, state=laststate, context=self.context
            )
        ...
        while contentcopy:
            ...
            if isinstance(e, str):
                ...
                if tagname != "dd" and tagname != "dt":
                    raise ValueError(
                        "don't expect %s here inside list, expect 'dd' or 'dt'" % repr(tagname)
                    )
                if tagname == "dt":
                    if bullet:
                        raise ValueError(
...
def Paragraph(text, style, bulletText=None, frags=None, context=None):
    ...
    if "&" not in text and "<" not in text:
        ...
        return Para(
            style, parsedText=parsedpara, bulletText=bulletText, state=None, context=context
        )
