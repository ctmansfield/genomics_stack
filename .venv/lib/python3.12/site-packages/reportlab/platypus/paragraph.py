...
def split(text, delim=None):
    ...
    return [
        uword
        for uword in (
            _wsc_re_split(text) if delim is None and "\xa0" in text else text.split(delim)
        )
    ]
            ...
            if kind == "img":
                ...
                if kind == "anchor":
                    ...
                    tx._canvas._curr_tx_info = dict(
                        tx=tx, cur_x=cur_x, cur_y=cur_y, leading=leading, xs=tx.XtraState
                    )
                    try:
...
class _SHYWord(list):
    ...
    def shyphenate(self, newWidth, maxWidth):
        ...
        R = (
            [ww - L[0] + shyLen]
            + ([] if not right else [(f, _SHYIndexedStr(right, [_ - x for _ in X[j + 1 :]]))])
            + self[i1 + 1 :]
        )
...
def _uri_split_pairs(uri):
    ...
    slash = "\\" if not scheme and "/" not in uri else "/"  # might be a microsoft pattern
...
def cjkFragSplit(frags, maxWidths, calcBounds, encoding="utf8"):
    ...
    while i < nU:
        ...
        if endLine:
            ...
            lines.append(
                makeCJKParaLine(
                    U[lineStartPos:i], maxWidth, widthUsed, extraSpace, lineBreak, calcBounds
                )
            )
            try:
    ...
    if widthUsed > 0:
        lines.append(
            makeCJKParaLine(
                U[lineStartPos:], maxWidth, widthUsed, maxWidth - widthUsed, False, calcBounds
            )
        )
...
class Paragraph(Flowable):
    ...
    def __init__(
        self, text, style=None, bulletText=None, frags=None, caseSensitive=1, encoding="utf8"
    ):
        if style is None:
    ...
    def breakLines(self, width):
        ...
        if nFrags == 1 and not (
            ...
            if hasattr(f, "text"):
                ...
                if not text:
                    return f.clone(
                        kind=0, lines=[], ascent=ascent, descent=descent, fontSize=fontSize
                    )
                else:
                    ...
                    if doBidi:
                        ...
                        if shaping:
                            ...
                            words = [
                                innerBidiStrWrap(
                                    shapeStr(_, fontName=f.fontName, fontSize=v.fontSize), VX[i], i
                                )
            ...
            while _words:
                ...
                if (
                    ...
                    if isinstance(w, _SHYWord):
                        ...
                        if hsw:
                            _words[0:0] = hsw
                            _words.insert(
                                1, _InjectedFrag([0, (f.clone(_fkind=_FK_BREAK, text=""), "")])
                            )
                        ...
                        if hsw:
                            ...
                            if hsw:
                                _words[0:0] = [
                                    _InjectedFrag([0, (f.clone(_fkind=_FK_BREAK, text=""), "")]),
                                    w,
                                ]
                                FW.pop(-1)  # remove this as we are doing this one again
                                continue
                    ...
                    if splitLongWords and not isinstance(w, _SplitFrag):
                        ...
                        if wordWidth > max(maxWidths[nmw : nmw + 1]):
                            ...
                            _words[0:0] = _splitFragWord(
                                w, maxWidth - spaceWidth - currentWidth, maxWidths, lineno
                            )
                            _words.insert(
                                1, _InjectedFrag([0, (f.clone(_fkind=_FK_BREAK, text=""), "")])
                            )
                ...
                if not endLine:
                    ...
                    if calcBounds:
                        if f._fkind == _FK_IMG:
                            descent, ascent = imgVRange(
                                imgNormV(f.cbDefn.height, fontSize), f.cbDefn.valign, fontSize
                            )
                        else:
                            ascent, descent = getAscentDescent(f.fontName, fontSize)
                    else:
                    ...
                    spaceWidth = (
                        stringWidth(" ", fontName, fontSize) if isinstance(w, _HSFrag) else 0
                    ...
                    for i in w[2:]:
                        ...
                        if calcBounds:
                            if g._fkind == _FK_IMG:
                                descent, ascent = imgVRange(
                                    imgNormV(g.cbDefn.height, fontSize), g.cbDefn.valign, fontSize
                                )
                            else:
        ...
        for i, f in enumerate(frags):
            if full:
                ...
                aR(
                    ...
                    % (
                        f[0],
                        ", ".join(("(%s,%r)" % (fx[0].__class__.__name__, fx[1]) for fx in f[1:])),
                        i,
...
umfassend zu sein."""
    ...
    if flagged(7):
        text = (
            """<para align="CENTER" fontSize="24" leading="30"><b>Generated by:</b>Dilbert</para>"""
        )
        P = Paragraph(text, styleSheet["Code"])
        dumpParagraphFrags(P)
        w, h = P.wrap(6 * 72, 9.7 * 72)
