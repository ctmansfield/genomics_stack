        ...
        if not (nrows and ncols):
            if emptyTableAction == "error":
                ...
                Preformatted.__init__(
                    self, "%s(%d,%d)" % (self.__class__.__name__, nrows, ncols), _emptyTableStyle
                )
...
only rows may be strings with values in {_SPECIALROWS!r}"""
        ...
        if isinstance(value, (tuple, list)):
            ...
            for flowable, _fh in zip(value, FH):
                ...
                if split:
                    ...
                    usedHeight += (
                        flowable_height + flowable.getSpaceBefore() + flowable.getSpaceAfter()
                    )
                else:
        ...
        if not doInRowSplit:
            ...
            if (minSplit + maxSplit > curRowHeight) or (minSplit > (availHeight - usedHeights)):
                ...
                for cell in splitCells:
                    ...
                    res = self._splitCell(
                        self._cellvalues[span_sr][span_sc], oldStyle, oldHeight, newHeight, width
                    )
                    if not res:
                        # Could not split
                        return []
    ...
    def _restoreRoundingObscuredLines(self):
        ...
        for xs, ys, xe, ye, weight, color, cap, dash, join in SL:
            ...
            if ys == ye:
                ...
                c0 = (
                    _quadrantDef("left", ypos, (xs, ys), r0, kind=2, direction="left-right")
                    if xs < x0 + r0
                    else None
                )
                c1 = (
                    _quadrantDef("right", ypos, (xe, ye), r1, kind=1, direction="left-right")
                    if xe > x1 - r1
                    else None
                )
            else:
                ...
                # we have some rounding so we must use a path
                c0 = (
                    _quadrantDef(xpos, "bottom", (xs, ys), r0, kind=2, direction="bottom-top")
                    if ys < y0 + r0
                    else None
                )
                c1 = (
                    _quadrantDef(xpos, "top", (xe, ye), r1, kind=1, direction="bottom-top")
                    if ye > y1 - r1
                    else None
                )
            P = canv.beginPath()
        ...
        if not self._spanCmds:
            ...
            if renderCB:
                for rowNo, (row, rowstyle, rowpos, rowheight) in enumerate(
                    zip(
                        self._cellvalues, self._cellStyles, self._rowpositions[1:], self._rowHeights
                    )
                ):
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if isinstance(cellval, (tuple, list, Flowable)):
            ...
            for v, w, h in zip(cellval, W, H):
                if just == "LEFT":
                    ...
                    x = (
                        colpos
                        + (colwidth + cellstyle.leftPadding - cellstyle.rightPadding - w) / 2.0
                    )
                else:
            ...
            if valign == "BOTTOM":
                ...
                y = (
                    rowpos
                    + (cellstyle.bottomPadding + rowheight - cellstyle.topPadding + n * leading)
                    / 2.0
                    - fontsize
                )
            else:
        ...
        if cellstyle.href:
            # external hyperlink
            self.canv.linkURL(
                cellstyle.href, (colpos, rowpos, colpos + colwidth, rowpos + rowheight), relative=1
            )
...
GRID_STYLE = TableStyle(
    [("GRID", (0, 0), (-1, -1), 0.25, colors.black), ("ALIGN", (1, 1), (-1, -1), "RIGHT")]
)
BOX_STYLE = TableStyle(
    [("BOX", (0, 0), (-1, -1), 0.50, colors.black), ("ALIGN", (1, 1), (-1, -1), "RIGHT")]
)
