...
class FrameActionFlowable(Flowable):
    ...
    def __init__(self, *arg, **kw):
        raise NotImplementedError(
            "%s.__init__ should never be called for abstract Class" % self.__class__.__name__
        )

    def frameAction(self, frame):
        raise NotImplementedError(
            "%s.frameAction should never be called for abstract Class" % self.__class__.__name__
        )
...
def _ktAllow(f):
    ...
    return not (
        isinstance(f, (_ContainerSpace, DocIf, DocWhile)) or getattr(f, "locChanger", False)
    )
...
class BaseDocTemplate:
    ...
    def __repr__(self):
        return (
            f"<{self.__class__.__module__}.{self.__class__.__name__} object at 0x?hidden?>"
            if invariant
            else super().__repr__()
        )
...
class SimpleDocTemplate(BaseDocTemplate):
    ...
    def build(
        self, flowables, onFirstPage=_doNothing, onLaterPages=_doNothing, canvasmaker=canvas.Canvas
    ):
        ...
        self.addPageTemplates(
            [
                ...
                PageTemplate(
                    id="Later", frames=frameT, onPage=onLaterPages, pagesize=self.pagesize
                ),
            ]
        )
