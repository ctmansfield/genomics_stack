...
def gitrepo(root):
    ...
    oldwd = os.getcwd()
    os.chdir(root)
    gitlog = git("--no-pager", "log", "-1", '--pretty="format:%s"' % FORMAT).split("\n", 7)
    branch = os.environ.get("CIRCLE_BRANCH") or os.environ.get(
        "TRAVIS_BRANCH", git("rev-parse", "--abbrev-ref", "HEAD").strip()
    )
    remotes = [x.split() for x in git("remote", "-v").strip().splitlines() if x.endswith("(fetch)")]
    os.chdir(oldwd)
    return {
        "head": {
            "id": gitlog[0],
