...
def parse_args():
    ...
    parser.add_argument(
        "path",
        help="Directory containing source files to " "analyze, or multiple file paths",
        nargs="+",
    )
    parser.add_argument(
        "-a",
    ...
    parser.add_argument(
        ...
        dest="absolute",
        metavar="<str>",
        help="Absolute threshold for block " "complexity",
    )
    parser.add_argument(
        "-e",
        "--exclude",
    ...
    parser.add_argument(
        ...
        metavar="<URL>",
        dest="url",
        help="Where to send the JSON data through a POST " "request.",
    )
    parser.add_argument(
        "--no-assert",
        dest="no_assert",
        action="store_true",
        help="Do not count `assert` statements when computing " "complexity",
    )
    parser.add_argument(
        "-c",
        "--config-file",
        ...
        dest="config",
        default=".xenon.yml",
        help="Xenon config file " "(default: %(default)s)",
    )
    parser.add_argument(
        "--paths-in-front",
        dest="paths_in_front",
    ...
    except (getattr(__builtins__, "FileNotFoundError", IOError), yaml.YAMLError):
        yml = {}
    args.repo_token = yml.get("repo_token", os.environ.get("BARIUM_REPO_TOKEN", ""))
